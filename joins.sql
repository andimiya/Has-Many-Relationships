...
--   FROM users
--   INNER JOIN posts ON posts.users_id = users.id
--     WHERE users.created_at > TIMESTAMP WITH TIME ZONE '2015-01-01 00:00:00-10';
...
--   INNER JOIN posts ON posts.users_id = users.id
--   INNER JOIN comments ON comments.posts_id = posts.id
--     WHERE posts.created_at < TIMESTAMP WITH TIME ZONE '2015-01-01 00:00:00-10';
...
--   INNER JOIN posts ON posts.users_id = users.id
--   INNER JOIN comments ON comments.posts_id = posts.id
--     WHERE users.username = comments.id;

--Additional Query #2
SELECT comments.created_at, comments.body
  FROM users
  INNER JOIN posts ON posts.users_id = users.id
  INNER JOIN comments ON comments.posts_id = posts.id
